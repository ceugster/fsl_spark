...
//	private static Logger logger = LoggerFactory.getLogger(Camt.class);
	...
	public void convertCamt(ObjectNode requestNode, ObjectNode responseNode)
	{
...
//	}
	...
	private boolean convertXmlToJson(String xml)
	{
		...
		try
		{
			...
			if (Objects.nonNull(mx))
			{
				...
				responseNode.put(Executor.RESULT, json);
				responseNode.put(IDENTIFIER_KEY, identifier);
			}
			else
			{
	...
	private boolean convertJsonToXml(String json)
	{
		...
		try
		{
			...
			if (Objects.nonNull(mx))
			{
				...
				String identifier = extractIdentifier(json);
				responseNode.put(Executor.RESULT, xml);
				responseNode.put(IDENTIFIER_KEY, identifier);
			}
			else
			{
	...
	private String extractIdentifier(String json)
	{
		...
		try
		{
			...
			JsonNode identifierNode = jsonNode.findPath(IDENTIFIER_KEY);
